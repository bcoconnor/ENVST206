...
#question 4
rgbS2 <- stack(rdatB3,rdatB2,rdatB4)/10000
#view raster, a few pixels in blue have reflectance above 1 so set scale
plotRGB(rgbS2, scale=2)
#don't need the scale argument when adding in the contrast stretch
plotRGB(rgbS2, stretch="lin")
...
NDVIraster <- cbind(landExtract,landcoverpoints)

head(NDVIraster)

ggplot(data=NDVIraster[NDVIraster$landcID==c("agri","forest","wetland"),], 
       aes(x=landcID, y=landcoverpoints), fill=landcoverpoints)+
